...
// Relu et mis en forme; pas de problème relevé
...
var swfobject=function()
{
	...
	R="SWFObjectExprInst",x="onreadystatechange",O=window,j=document,t=navigator,T=false,U=[h],o=[],N=[],I=[],l,Q,E,B,J=false,
	a=false,n,G,m=true,
	
	M=function(){
		var aa=typeof j.getElementById!=D&&typeof j.getElementsByTagName!=D&&typeof j.createElement!=D,
		ah=t.userAgent.toLowerCase(),Y=t.platform.toLowerCase(),ae=Y?/win/.test(Y):/win/.test(ah),ac=Y?/mac/.test(Y):/mac/.test(ah),
		af=/webkit/.test(ah)?parseFloat(ah.replace(/^.*webkit\/(\d+(\.\d+)?).*$/,"$1")):false,X=!+"\v1",ag=[0,0,0],ab=null;
	...
	k=function(){
		...
		if(!J){
			...
			if(M.ie&&M.win){
				j.attachEvent(x,function(){if(j.readyState=="complete"){j.detachEvent(x,arguments.callee);f()}});
				if(O==top){
		...
		if(Z){
			...
			(function(){
				if(typeof Z.GetVariable!=D){
					...
					if(Y<10){Y++;setTimeout(arguments.callee,10);return}
				}
	...
	function P(aa,ab,X,Z){
		...
		if(ae){
			if(ae.nodeName=="OBJECT"){l=g(ae);Q=null}
			else{l=ae;Q=X}
			...
			if(M.w3&&!(M.wk&&M.wk<312)&&ab&&ah&&ae&&ag&&Y){
				...
				K(function(){
					...
					if(F(Y)){
						...
						if(aa&&A()){aj.data=aa;P(aj,am,ah,ac);return}
						else{w(ah,true)}
					}
					if(ac){ac(X)}
				})
